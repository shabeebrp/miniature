...
func Map(filename string, contents string) []worker.KeyValue {
	...
	kva := []worker.KeyValue{}
	kva = append(kva, worker.KeyValue{"a", "1"})
	kva = append(kva, worker.KeyValue{"b", "1"})
	kva = append(kva, worker.KeyValue{"c", "1"})
	kva = append(kva, worker.KeyValue{"d", "1"})
	kva = append(kva, worker.KeyValue{"e", "1"})
	kva = append(kva, worker.KeyValue{"f", "1"})
	kva = append(kva, worker.KeyValue{"g", "1"})
	kva = append(kva, worker.KeyValue{"h", "1"})
	kva = append(kva, worker.KeyValue{"i", "1"})
	kva = append(kva, worker.KeyValue{"j", "1"})
	return kva
}
